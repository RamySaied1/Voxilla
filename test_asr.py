...
# python3 test_asr.py -model ./Experiments/BLSTM1/BLSTM_CE -fst_folder ./Decoder/Graphs/200k-vocab/ -input_labels_file labels.ciphones -srcfile waves_test.txt -outfile transcript
def main():
    ...
    parser.add_argument('-input_labels_file', '--input_labels_file', help="Text file name containing input labels", required=True, default=None)
    ...
    asr = ASR(args.model, args.fst_folder, args.input_labels_file)
        ...
        with open(args.srcfile, 'r') as ftest:
            with open(args.outfile, 'w', buffering=1) as fout:
                ...
                for line in ftest.read().split():
                    text = asr.speech_to_text(line,1000,12,3)
