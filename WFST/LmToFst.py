...
def main():
    ...
    lm = LmToFst(args.lmfile)
    lm.make_fst(args.prune_5k)
    lm.write_fst_body(args.out_file_base + '.txt')
...
class LmToFst:
    def __init__(self, filename):
        ...
        def record_ngram(line: str):
            ...
            backoff_score = float(parts[current_ngram + 1] if len(parts) > current_ngram + 1 else 0)  # 0?!?! why not  -99
    ...
    def make_fst(self, prune_5k=False):
        ...
        for ngram in self.ngrams:
            ...
            label = word
            if score[0] > -99:  # this number means 0 probabilty set by SLRM library
                arcs.append((src, dst, word, label,-np.log10(np.exp(1)) * score[0]))
                # what does this mean -np.log10(np.exp(1)) * score[0])

            # insert the backoff arc
            if len(ngram) < self.max_ngram and word != endSym:
